...
demand_fixed.df = read.csv( "input/maragheh_demands.csv", stringsAsFactors=FALSE )

# capacity of technologies
hist_new_cap.df = read.csv( "input/new_cap.csv", stringsAsFactors=FALSE ) %>%
					dplyr::select( -units )
...
#basin_water_connections.df = read.csv( 'input/maragheh_water_connections_sdg.csv', stringsAsFactors=FALSE ) %>%
#								filter( SSP == 'SSP2' )
...
# from the 3 western rivers (maragheh, Chenab, and Jehlum) and 10 km3
...
# transmission costs million USD per MW, see file maragheh_transmission_cost.xlsx - cost of 1.31 USD / kW per km -  in line with WECC data from SWITCH model
...
residue_data = data.frame(crop = c('wheat','fodder','pulses','fruit'),
                          res_yield = c(5e3 , 8.4e3 , 6.48e3 , 6.7e3 ), #kt/Mha
                          mode = c(1:7),
