# check trends and prinf pdfs in D:/GIS/IIASA/Git/inputs/Sufi_chai/Sufi_chai
library(ggplot2)
#####
# check total runoff
...
runoff_by_admin.df = rbind( runoff_by_admin.df, runoff_by_admin.df %>%
                              ...
                              mutate( country = 'sufi_chai' ) %>% ungroup() %>% data.frame() %>% select( year_all, time, country, value ) ) 
...
plot.new()
legend( 'left', legend = c(2016,seq(2020,2060,by=10) ), title = 'Decade', bty = 'n', cex = 1.2, seg.len = 3, col = c('black','cyan','blue','purple','brown','red'), lty = 1 )	
dev.off()
###
...
for( ccc in unique( yield_by_admin.df$country ) ){
  df = yield_by_admin.df %>% filter( country == ccc ) %>%
    filter( year_all %in% c(2016, seq(2020, 2060, by = 10) ) ) %>%
    select( year_all, commodity, value ) %>%
    mutate( value = value / 1e3 ) %>%
    reshape( ., idvar = 'commodity', timevar = 'year_all', direction = 'wide' ) 
  barplot( as.matrix( df %>% select( -commodity ) ), ylab = 'million tons', names.arg = c(2016, seq(2020, 2060, by = 10) ), las = 2, main = paste0( ccc, ' - Yield' ), col = as.character(cols[ df$commodity, ]) )
  abline( h = 0 )
}
plot.new()
...
#### check water supply>demand in 2016
## Run load_inputs first

inflow2015 = inflow.df %>% filter(year_all == 2016) %>% 
  rename(inflow = value) %>% mutate(time = as.character(time)) %>% 
  select(node,year_all,time,inflow)

return2015 = demand.df %>% filter(commodity == 'wastewater', year_all == 2016) %>% 
...
UR_demand2015 = demand.df %>% filter(commodity == 'freshwater', level %in% c('urban_final','rural_final'), year_all == 2016) %>% 
...
for (jj in seq_along(nat_trend.files)){
  ...
  tmp_national = read.csv(paste0(national_trend.path,'/',nat_trend.files[jj])) %>% 
    filter(Unit == 'tonnes', Year <= 2016 ) %>% 
    mutate(Value = Value * 1E-6, Unit = 'mton') %>% 
    left_join(data.frame(Area = c('Maragheh'),country = c('Maragheh'))) %>% 
    left_join(data.frame(Item = c( 'Pulses', 'fodder', 'Wheat','fruits' ),crop = crop_names)) %>% 
    select(country,crop,Year,Unit,Value) %>% 
    rename(year_all = Year, unit = Unit, value = Value)
  prod_hist = bind_rows(prod_hist,tmp_national)
}
yield_by_admin.df2 = yield_by_admin.df %>% mutate(unit = 'Mton') %>% filter(!country == 'sufi_chai') %>% rename(crop = commodity) %>% 
  select(country,crop,year_all,unit,value) %>% mutate(value = value/1000)
  # rbind(prod_hist) %>% 
  # arrange(year_all)

# from crop_yield, take prod_out, production in 2016
prod_out_sum = prod_out %>% mutate(country = gsub('_.*','',node)) %>% 
  group_by(country,crop,unit) %>% 
  summarise(value = sum(value)/1000) %>% ungroup() %>% 
  mutate(year_all = 2016,unit = 'Mton')

p11 = ggplot()+
  geom_line(data = yield_by_admin.df2 %>% filter(country == 'Maragheh'),aes(year_all,value,colour = crop))+
  geom_line(data = prod_hist %>% filter(country == 'Maragheh'),aes(year_all,value,colour = crop))+
  geom_point(data = prod_out_sum %>% filter(country == 'Maragheh'),aes(year_all,value,colour = crop),size = 2)+
  geom_vline(xintercept=2016)+
  ylab('Maragheh Production [Mton]')+
  theme_bw()+
...
p1 = ggplot(impo_exp %>% filter(item %in% c('Pulses', 'fodder', 'Wheat','fruits')))+
  geom_line(aes(x = year,y = value,color = item, linetype = element),size = 1)+
  facet_wrap(~country)+
  scale_color_brewer(type = 'qual',palette = 2)+
  theme_bw()

p2 = ggplot(impo_exp %>% filter(!item %in% c('Pulses', 'fodder', 'Wheat','fruits')))+
  geom_line(aes(x = year,y = value,color = item, linetype = element),size = 1)+
  facet_wrap(~country)+
...
level_dams = read_csv("D:/GIS/IIASA/Git/inputs/Sufi_chai/Sufi_chai/input/storage/sufi_chai_reservoir_levels.csv")
