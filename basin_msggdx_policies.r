...
if (REDUCE_RUNOFF){ demand.par = demand.par %>% 
  mutate( value = if_else(level == 'river_in' & grepl('Maragheh',node),value * (1 - percent_reduction / 100 ), value ) ) } 
...
# Simplified approach: Bound irrigation withdrawals to historical levels in the sub-basins by the policy
...
bound_activity_lo.par = bind_rows( bound_activity_lo.par, 
	...
	# sufi_chaitry wastewater treatment rate
	do.call( rbind, lapply( nds, function( nnn ){
		...
		} ) ) %>% mutate(tec = 'sufi_chaitry_wastewater_collection') %>% dplyr::select( node, tec, year_all, mode, time, value ),	
	...
	do.call( rbind, lapply( node[ node != basin ], function( nnn ){
		...
		do.call( rbind, lapply( year_all[ which( year_all > baseyear ) ], function( yyy ){ 
			...
			do.call( rbind, lapply( time, function( mmm ){
				...
				data.frame( node = nnn, 
							tec = 'sufi_chaitry_wastewater_recycling',
							year_all = yyy, 
							mode = 1,
							time = as.character( mmm ),
							value = abs( round( 	0.1 * min( 	0, 
														demand.par %>% 
															dplyr::filter( node == nnn, commodity == 'wastewater', level == 'sufi_chaitry_final', year_all == yyy, time == mmm ) %>%
															dplyr::select( value ) %>% unlist() ,
														na.rm = TRUE ), digits = 3 ) ) )
...
if (!EMISS){} else{
	...
	tmp = rgdx( paste( paste( sufi_chai_ix_path, '/model/output/', sep=''), 
